...
class CodableFeedStoreTests: XCTestCase {
    ...
    func test_insert_overridesPreviouslyInsertedCacheValues() {
        ...
        let firstInsertionError = insert(cache: (uniqueImageFeed().locals, Date()), to: sut)
    ...
    //MARK: - CACHE DELETION TESTS
    
    func test_delete_deliversEmptyOnDeletingEmptyCache() {
        let sut = makeSUT()
        
        let deletionError = deleteCache(from: sut)
        XCTAssertNil(deletionError, "Expect to delete the cache")
        
        expect(sut: sut, toRetrieve: .empty)
    }
    
    func test_delete_deliversEmptyOnDeletingNonEmptyCache() {
        let sut = makeSUT()
        let feed = uniqueImageFeed().locals
        let timestamp = Date()
        insert(cache: (feed, timestamp), to: sut)
    ...
    func test_delete_deliversErrorOnDeletionError() {
        ...
        XCTAssertNotNil(deletionError, "Expect deletionError to be non-nil")
        expect(sut: sut, toRetrieve: .empty)
    }
